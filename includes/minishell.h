...
/*   Updated: 2023/01/22 16:48:20 by bbonaldi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token			*ft_find_last_token(t_token *head);
char			*ft_advance_str_ptr_exp_handler(char **str,
					char *start_var_expression, size_t len[2]);
...
void			ft_print_custom_error_and_exit(t_ms *ms, char *argv,
					char *message, int exit_code);
void			ft_print_custom_error_and_exit2(t_ms *ms, char *argv[2],
					char *message, int exit_code);
...
void			ft_set_syntax_error_and_exit_code(t_ms *ms,
					t_error_type error_type, int exit_code);
...
t_bool			should_stop_on_signals_quoting(t_ms *ms, char *line,
					char quote);
