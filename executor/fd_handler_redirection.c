...
/*   Updated: 2023/01/22 16:23:11 by bbonaldi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	ft_init_redirection_fds(t_ms *ms, t_file *file, t_bool is_child)
{
	int		flags;
	...
	if (file->fd < 0)
	{
		...
		if (is_child)
			...
			ft_print_error_and_invalid_program(ms, file->file);
	}
	return (file->fd);
}
...
void	ft_set_redirection_fds(t_ms *ms, t_executor *exec_tree, t_bool is_child)
{
	int		fds[2];
	t_file	*file;
	t_list	*file_list;
	...
	while (file_list)
	{
		...
		if (file->type == INFILE || file->type == HEREDOC)
			fds[INFILE] = ft_init_redirection_fds(ms, file, is_child);
		else if (file->type == TRUNCATE || file->type == APPEND)
			fds[OUTFILE] = ft_init_redirection_fds(ms, file, is_child);
		file_list = file_list->next;
